...
export function ActivityTimeline({
  ...
  const sensors = useSensors(
    // Small activation distance keeps taps on touch devices working.
    useSensor(PointerSensor, { activationConstraint: { distance: 6 } }),
    // Arrow-key reordering when a grip handle is focused.
    useSensor(KeyboardSensor, { coordinateGetter: sortableKeyboardCoordinates }),
  );

  /* Overlap pairs keyed per activity for inline warnings. */
  const overlapsByActivity = useMemo(() => {
    const map = new Map<string, Array<{ otherName: string; startTime: string; endTime: string; activityId: string; otherId: string }>>();
    for (const [a, b] of dayOverlapPairs(activities)) {
      if (!map.has(a.id)) map.set(a.id, []);
      if (!map.has(b.id)) map.set(b.id, []);
      map.get(a.id)!.push({ activityId: a.id, otherId: b.id, otherName: b.name, startTime: b.startTime, endTime: b.endTime });
      map.get(b.id)!.push({ activityId: b.id, otherId: a.id, otherName: a.name, startTime: a.startTime, endTime: a.endTime });
    }
    return map;
  }, [activities]);
    ...
    >
      ...
      <DragOverlay>
        {draggingActivity ? (
          <div
            className={cn(
              "rounded-xl border border-primary/40 bg-card px-4 py-3 text-sm font-medium text-foreground shadow-lg",
            )}
          >
      ...
      {/* Screen-reader instructions for keyboard drag */}
      <p className="sr-only">
      ...
      {/* Quick add at the bottom of every timeline */}
